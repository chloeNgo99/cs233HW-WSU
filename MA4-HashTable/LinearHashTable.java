...
class LinearHashTable<K, V> extends HashTableBase<K, V>
{
    ...
    public void addElement(K key, V value)
    {
        ...
        //HashItem<K, V> slot = new HashItem<K,V>(key, value);
        int i = 1;
        while(!_items.get(hash).isEmpty() && !_items.get(hash).getKey().equals(key)) {
        	hash = (hash + i) % _items.size();
        	i++;
        }
        if(_items.get(hash).isEmpty()) {
        	_items.get(hash).setKey(key);
        	_items.get(hash).setValue(value);
        	_items.get(hash).setIsEmpty(false);
        }
    ...
    public void removeElement(K key)
    {
        ...
        while(!_items.get(hash).isEmpty()) {
        	if(_items.get(hash).getKey().equals(key)) {
        		_items.get(hash).setIsEmpty(true);
        		_number_of_elements--;
        	}
        	hash = (hash + i)% _items.size();
        	i++;
