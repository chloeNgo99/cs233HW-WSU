...
>>>>>>> dce82e5457ba3421c4037fd789a7aedf09c860fb
		...
		// Return the new root
		System.out.println("root right rotation "+root.getValue());
		AvlNode<T> left = root;
		if(root.getLeftChild() != null) {
			left = root.getLeftChild();
			root.setLeftChild(left.getRightChild());
			left.setRightChild(root);
			root.setHeight(Math.max(calcNodeHeight(root.getRightChild()), calcNodeHeight(root.getLeftChild()))+1);
			left.setHeight(Math.max(calcNodeHeight(left.getRightChild()), calcNodeHeight(root)) + 1);
		}
...
>>>>>>> dce82e5457ba3421c4037fd789a7aedf09c860fb
		...
		// And perform additional balances as necessary
		// return the new root
		System.out.println("root left rotation "+root.getValue());
		AvlNode<T> right = root;
		if(root.getRightChild() != null) {
			right = root.getRightChild();
			root.setRightChild(right.getLeftChild());
			right.setLeftChild(root);
			root.setHeight(Math.max(calcNodeHeight(root.getLeftChild()), calcNodeHeight(root.getRightChild()))+1);
			right.setHeight(Math.max(calcNodeHeight(right.getLeftChild()), calcNodeHeight(root)) + 1);
		}
	...
	protected AvlNode<T> balance(AvlNode<T> root)
	{
		...
		// Return updated root
		int balance = calcNodeHeight(root.getLeftChild()) - calcNodeHeight(root.getRightChild());
		//left heavy
		if(balance > 1) {
			balance = calcNodeHeight(root.getLeftChild().getLeftChild()) - calcNodeHeight(root.getLeftChild().getRightChild());
			if(balance > 0) {
		...
		//right heavy
		if(balance < -1) {
			balance = calcNodeHeight(root.getRightChild().getLeftChild()) - calcNodeHeight(root.getRightChild().getRightChild());
			if(balance < 0) {
